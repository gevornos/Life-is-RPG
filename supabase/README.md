...
### 1.5. (Опционально) Функция удаления аккаунта

Для тестирования добавь функцию, которая позволяет удалять свой аккаунт:

1. В **SQL Editor** создай новый запрос
2. Скопируй содержимое файла `delete-user-function.sql`
3. Выполни (Run)

Это позволит кнопке "Удалить аккаунт полностью" в приложении действительно удалять пользователя из базы, а не просто выходить из аккаунта.
